...
def train_epoch(model, train_loader, criterion, optimizer, device):
    ...
    train_hamming_loss = hamming_loss(all_labels, all_predictions)

    return train_loss, train_accuracy, train_accuracy_ones, train_f1_score, train_hamming_loss
...
def validate_epoch(model, valid_loader, criterion, device, threshold=0.5):
    ...
    valid_hamming_loss = hamming_loss(all_labels, all_predictions)

    return valid_loss, valid_accuracy, valid_accuracy_ones, valid_f1_score, valid_hamming_loss
...
if __name__ == "__main__":
    ...
    optimizer = optim.Adam(model.parameters(), lr=0.001, weight_decay=0.00697)
    ...
    for epoch in range(num_epochs):
        train_loss, train_accuracy, train_ones_accuracy, train_f1, train_hamming = train_epoch(model, train_loader, criterion, optimizer, device)
        valid_loss, valid_accuracy, valid_ones_accuracy, valid_f1, valid_hamming = validate_epoch(model, valid_loader, criterion, device)
        ...
        print(f"Epoch {epoch+1}: Train Loss: {train_loss:.4f}, Train Hamming: {train_hamming:.4f}, Train One Acc: {train_ones_accuracy:.4f}, Train F1: {train_f1:.4f}, Val Loss: {valid_loss:.4f}, Val Hamming: {valid_hamming:.4f}, Val One Acc: {valid_ones_accuracy:.4f}, Val F1: {valid_f1:.4f}")
    ...
    axs[3].plot(train_hamming_losses, label='Training Hamming Loss')
    axs[3].plot(valid_hamming_losses, label='Validation Hamming Loss')
    axs[3].set_xlabel('Epoch')
    axs[3].set_ylabel('Hamming Loss')
    axs[3].legend(loc='upper right')
    axs[3].set_title('Training vs Validation Hamming Loss')
